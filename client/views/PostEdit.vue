<template>
<header>
    <h2 class="color">Edit Post</h2>
</header>
<template v-if='!(post?.loading || post?.error)'>
    <Form ref="editForm" @submit='onSubmit' :validation-schema='schema' :initial-values='post' v-slot='{ setFieldValue, errors, isSubmitting }' id='EditForm' v-if="post">
        <div class="formGroup">
            <label>Cover</label>
...
const updatedPost = { title: 'UPDATED TITLE', body: 'UPDATED BODY', preview: 'UPDATED PREVIEW', tags: 'NATURE, Religion, Computer Science' };
...
async function onSubmit(values) {
    await postStore.update(values);

    if (postStore.error) {
        emitBus('notify', {
            type: 'error',
            title: `Error updating the post (${postStore.error})`,
